...
func main() {
	...
	var p1 *int
	var p4array *[3]int
	fmt.Println(p1, p4array)

	var n int = 100
	//アドレス演算子"&"をつけると実際の番地を指す。
	fmt.Println(n, &n)    // n: 100 &n: 0xな番地
	ret1 := DoubleFunc(n) //引数を2倍する関数を呼んでも、基本型は値渡しなので変わらない。
	fmt.Println(ret1)
	...
	*p = 300           //ポインタでデリファレンスし、指している実体を書き換える
	...
	var mi1 MyInt = 5
	//type Strings []string              //Strings{"aaa", "bbb"}
	...
	pFromConstructor := NewPoint(1, 2) // *Point型で生成される
	pFromConstructor.X = 1000
